                    ...
                    do
                    {
                        ...
                        else if (found && !with_value(outfile, NULL, expect->with_value, expect->with_flags, found, buffer, sizeof(buffer)))
                        {
                                     ...
                                     !expect->repeat_match && (!expect->repeat_no_match || repeat_count >= expect->repeat_limit))
                            {
                                ...
                                else if (expect->with_flags & _CUPS_WITH_HOSTNAME)
                                {
                                    add_stringf(errors, "EXPECTED: %s %s \"%s\"",
                                                expect->name,
                                                (expect->with_flags & _CUPS_WITH_ALL) ?
                                                "WITH-ALL-HOSTNAMES" : "WITH-HOSTNAME",
                                                expect->with_value);
                                }
                                else if (expect->with_flags & _CUPS_WITH_RESOURCE)
                                {
                                    add_stringf(errors, "EXPECTED: %s %s \"%s\"",
                                                expect->name,
                                                (expect->with_flags & _CUPS_WITH_ALL) ?
                                                "WITH-ALL-RESOURCES" : "WITH-RESOURCE",
                                                expect->with_value);
                                }
                                else if (expect->with_flags & _CUPS_WITH_SCHEME)
                                {
                                    add_stringf(errors, "EXPECTED: %s %s \"%s\"",
                                                expect->name,
                                                (expect->with_flags & _CUPS_WITH_ALL) ?
                                                "WITH-ALL-SCHEMES" : "WITH-SCHEME",
                                                expect->with_value);
                                }
                                else
                                {
    ...
    switch (attr->value_tag)
    {
        ...
        case IPP_TAG_URISCHEME :
            ...
            else if (ippGetValueTag(attr) == IPP_TAG_URI)
            {
                ...
                if (flags & _CUPS_WITH_HOSTNAME ||
                    flags & _CUPS_WITH_RESOURCE ||
                    flags & _CUPS_WITH_SCHEME)
                {
                    /*
                     * "value" is a host name or a scheme or a resource path,
                     * and the attribute value contains 1+ URIs.
                     * See if the host name / scheme / resource path portion
                     * of the URI(s) match "value" in a case insensitive comparison...
                     */
